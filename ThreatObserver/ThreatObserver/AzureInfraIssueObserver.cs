    ...
    class AzureInfraIssueObserver : Actor<ThreatState>, IIssueObserver
    {
        public async Task<bool> Update(ThreatState value)
        {
            try
            {
                ...
                {
                    this.State = value;
                    Console.WriteLine(this.State.ExceptionType + " : " + this.State.ExceptionMessage + " " + this.State.TestRunId);
            ...
            {
                Trace.WriteLine("Exception seen in AzureInfraIssueObserver: {0}", ex.InnerException.Message);
                return false;
            }
            return true;
        }
